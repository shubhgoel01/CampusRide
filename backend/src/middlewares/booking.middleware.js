...
const newBookingHandler = asyncHandler(async (req, res, next) => {
  ...
  if (existingBooking)
    throw new ApiError(
      400,
      "You already have an active or returned booking awaiting verification. Please finish or wait for verification before creating a new booking",
    );
...
const endBookingHandler = asyncHandler(async (req, res, next) => {
  ...
  const booking = await Booking.findById(new mongoose.Types.ObjectId(bookingId))
    .select("userId estimatedEndTime")
    .lean();

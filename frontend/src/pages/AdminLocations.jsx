...
export default function AdminLocations({ onStatusNotice }) {
  ...
  const handleAdd = async () => {
    if (!name || lng === "" || lat === "") {
      onStatusNotice?.("error", "Please provide name, longitude and latitude");
      return;
    }
    try {
      await addLocation({
      ...
      onStatusNotice?.("success", "Location added successfully");
      setName("");
      setLng("");
      setLat("");
      loadLocations();
      ...
      onStatusNotice?.(
        "error",
        e?.response?.data?.message || "Failed to add location",
      );
    }
  };
  ...
  const handleDelete = async (locId) => {
    ...
    try {
      ...
      onStatusNotice?.("success", "Location deleted successfully");
      loadLocations();
    } catch (e) {
      onStatusNotice?.(
        "error",
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="w-full flex justify-center">
        <div className="w-full md:w-11/12 lg:w-10/12 bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-lg p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div>
              ...
              <input
                ...
                className="w-full mt-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-400 text-sm"
              />
            </div>
          ...
          <div className="mt-4 flex justify-end">
            <button
              onClick={handleAdd}
              className="px-6 py-2 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-purple-600 hover:to-blue-600 text-white transform hover:-translate-y-0.5"
            >
              Add Location
            </button>
          </div>
          ...
          {!loading &&
            locations.map((l) => (
              <div
                ...
                className="bg-white/10 rounded-2xl p-3 pb-6 shadow-md transform hover:shadow-lg hover:-translate-y-1 transition duration-150 ease-out flex flex-col justify-between cursor-pointer"
              >
                <div className="mb-2">
                ...
                <div className="flex items-center justify-end gap-2 flex-wrap">
                  <button
                    ...
                    className="px-2 py-1 border border-red-600 text-red-600 rounded hover:bg-red-600 hover:text-white transition text-sm whitespace-nowrap"
                  >
                    Remove
                  </button>
                </div>
      ...
      {viewModalOpen && (
        <Modal
          open={true}
          title={viewLocation?.name || "Location details"}
          onClose={() => {
        ...
        >
          <div className="text-sm">
            <div className="mb-2">
              <strong>Name:</strong> {viewLocation?.name || "-"}
            </div>
            <div className="mb-2">
              <strong>Longitude:</strong>{" "}
              {viewLocation?.longitude ??
                (viewLocation?.coordinates &&
                  viewLocation.coordinates?.coordinates &&
                  viewLocation.coordinates.coordinates[0]) ??
                "-"}
            </div>
            <div className="mb-2">
              ...
              {viewLocation?.latitude ??
                (viewLocation?.coordinates &&
                  viewLocation.coordinates?.coordinates &&
                  viewLocation.coordinates.coordinates[1]) ??
                "-"}
            </div>
            {viewLocation?.description && (
              <div className="mb-2">
                <strong>Description:</strong> {viewLocation.description}
              </div>
            )}
          </div>
        </Modal>

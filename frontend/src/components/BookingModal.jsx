...
export default function BookingModal({ open, booking, onClose }) {
  ...
  const getDuration = () => {
    ...
    if (booking.startTime && booking.actualEndTime) {
      const diff =
        new Date(booking.actualEndTime) - new Date(booking.startTime);
      ...
      <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 relative overflow-hidden">
        ...
        <div className="text-center mb-6">
          ...
          <div className="text-3xl font-bold text-slate-800">
            {booking.penaltyAmount && booking.penaltyAmount > 0
              ? `₹${(booking.penaltyAmount / 100).toFixed(2)}`
              : "₹0.00"}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-start">
            <span className="text-slate-400 text-xs uppercase font-bold mt-1">
              Start At
            </span>
            <div className="text-right">
              <div className="font-medium text-slate-800 text-sm">
                {renderLocationName(
                  booking.startLocationName,
              ...
              <div className="text-xs text-slate-400 font-mono">
                {booking.actualEndTime
                  ...
                  : booking.estimatedEndTime
                    ? `Est: ${formatTime(booking.estimatedEndTime)}`
                    : "—"}
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 text-center">
          ...
          <div className="flex justify-center gap-1">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="w-1 h-8 bg-slate-200"></div>
            ))}
          </div>
        </div>
      </div>
